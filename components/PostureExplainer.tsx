...
function TabContent({ posture }: { posture: Posture }) {
  return (
    <div
      className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-6 md:gap-8"
      style={{ animation: 'fadeIn 0.2s ease-out' }}
    >
      {/* Left: art + icon */}
      <div className="flex flex-col items-center md:items-start gap-4">
        <div className="w-full max-w-[240px] rounded-2xl overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
          <div className="h-1.5 w-full" style={{ backgroundColor: posture.colorVar }} />
          <img
            src={posture.art}
            alt=""
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h3 className="font-display font-medium text-xl text-ink leading-tight">
              {posture.name}
            </h3>
  ...
  return (
    <div className="w-full max-w-[900px]">
      ...
      <div className="relative mb-8">
        <div className="flex overflow-x-auto gap-1 pb-2 scrollbar-hide -mx-1 px-1" role="tablist">
          {POSTURES.map((p, i) => (
            <button
              key={p.id}
